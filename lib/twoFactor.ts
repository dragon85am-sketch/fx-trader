...
export function verifyTotp(secret:string, token:string){if(!/^\d{6}$/.test(token))return false;const c=Math.floor(Date.now()/30000);return [-2,-1,0,1,2].some(d=>hotp(secret,c+d)===token)}
