      ...
      async (tx: Prisma.TransactionClient) => {
        ...
        if (
          ...
          if (!user?.stripeAccountId) {
            throw new Error(
              "Użytkownik nie ma Stripe Connect"
            );
          }

          // Sprawdzenie konta Stripe Connect
          ...
          const payoutAmount = Math.round(
            Number(payout.amount) * 100
          );

          // Sprawdzenie dostępnego salda Stripe
          const balance = await stripe.balance.retrieve();

          const available = balance.available.find(
            (balanceItem) =>
              balanceItem.currency === payoutCurrency
          );

          if (
            !available ||
            available.amount < payoutAmount
          ) {
            throw new Error(
              "Brak środków na Stripe balance"
            );
          }
        ...
        if (
          ...
          await sendEmail({
            ...
            subject:
              "Wypłata affiliate została zrealizowana",
            ...
            html: `
              <div style="font-family:Arial,sans-serif;line-height:1.6">
                ...
                <p>
                  Cześć ${
                    payoutUser.name || "Partnerze"
                  },
                </p>

                <p>
                  Twoja wypłata afiliacyjna została
                  pomyślnie zrealizowana.
                </p>
